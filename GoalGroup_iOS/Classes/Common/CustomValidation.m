...
#import "Common.h"
@implementation CustomValidation
...
{
    NSUInteger strLength = value.length;
    BOOL bResult = TRUE;
    
    switch (symbol) {
        case LENGTH_EQUAL:
            bResult = (strLength == length);
            break;
        case LENGTH_GREAT:
            bResult = (strLength > length);
            break;
...
/**
 * bInclude:TRUE = Escape기호를 포함한다.
 * bInclude:False = Escape기호를 포함하지 않는다.
 *
 */
...
//    NSCharacterSet *nameCharacters = [[NSCharacterSet characterSetWithCharactersInString:@"_abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"] invertedSet];
    NSCharacterSet *nameCharacters = [NSCharacterSet characterSetWithCharactersInString:@"[`~!@#$%^&*()+=|{}':;',\\[\\].<>/?~！@#￥%……&*（）——+|{}【】‘；：”“’。，、？\t\f\n\r ]"];
    NSRange userNameRange = [value rangeOfCharacterFromSet:nameCharacters];
    
    if (userNameRange.location != NSNotFound) {
        NSLog(LANGUAGE(@"INCLUDE SPECIAL CAHR"));
        bResult = TRUE;
    }
    
    bResult = (bResult == bInclue);
    return bResult;
}
