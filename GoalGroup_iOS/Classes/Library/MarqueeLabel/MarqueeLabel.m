...
static CGFloat const _fadingLength = 0.0f;
...
//    [child release];
    ...
    [self setAnimationRate:8.0];
    [self setAnimationBorderDelay:1.0];
    [self setPaused:NO];
...
- (void)calculate
{
    ...
    [self setBeginPoint:CGPointMake( [self fadingLength] + [self bounds].size.width / 2 - [self fadingLength] + 4, [self bounds].size.height / 2 )];
