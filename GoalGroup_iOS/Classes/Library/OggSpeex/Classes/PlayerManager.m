...
- (void)playAudioWithFileName:(NSString *)filename delegate:(id<PlayingDelegate>)newDelegate {
    ...
    if ([filename rangeOfString:@".bcv"].location != NSNotFound || [filename rangeOfString:@".gga"].location != NSNotFound) {
        ...
        if ( ! [[NSFileManager defaultManager] fileExistsAtPath:filename]) {
            NSLog(@"%@:%@",LANGUAGE(@"DECAPSULATOR LOG"), filename);
...
- (void)startProximityMonitering {
    ...
    NSLog(LANGUAGE(@"ProxyMonitorEnabled"));
}

- (void)stopProximityMonitering {
    [[AVAudioSession sharedInstance] setCategory:AVAudioSessionCategoryPlayAndRecord error:nil];
    [[UIDevice currentDevice] setProximityMonitoringEnabled:NO];
    NSLog(LANGUAGE(@"ProxyMonitorDisabled"));
}
