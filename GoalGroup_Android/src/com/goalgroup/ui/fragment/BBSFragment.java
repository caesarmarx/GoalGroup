...
import com.goalgroup.task.GetBbsListTask;
...
public class BBSFragment extends BaseFragment
	...
	private GgListView bbsListView;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		View mainView = inflater.inflate(R.layout.layout_bbs, null);
		
		tvTitle = (TextView)mainView.findViewById(R.id.top_title);
		tvTitle.setText(R.string.bbs);
		
		adapter = new BbsListAdapter(getActivity(), BBSFragment.this);
		
		bbsListView = (GgListView)mainView.findViewById(R.id.bbs_listview);
		bbsListView.setPullLoadEnable(true);
		bbsListView.setPullRefreshEnable(false);
		bbsListView.setXListViewListener(this, 0);
	...
	public void onClick(View view) {
		...
		case R.id.social_mnu_add_bbs:
			...
			startActivityForResult(intent, ADD_BBS);
			break;
	...
	public void startAttachBbsList(boolean showdlg) {
		...
		GetBbsListTask task = new GetBbsListTask(this, adapter);
		task.execute();
	}
	
	public void stopAttachBbsList(boolean hasMore) {
		if (dlg != null) {
			dlg.dismiss();
			dlg = null;
		}
		...
		adapter.notifyDataSetChanged();
		bbsListView.setPullLoadEnable(hasMore);
	}
