 ...
 */
package com.goalgroup.chat.io.socket;
	...
	private class HearbeatTimeoutTask extends TimerTask {
		...
		public void run() {
			try {
				...
				connection.connect();
				ChatEngine.connect_flag = true;
			} catch (Exception e) {
				cleanup();
				ChatEngine.connect_flag = false;
			}
...
//					lastException));
	...
	private void handshake() {
		...
		try {
			...
			sessionId = data[0];
			heartbeatTimeout = Long.parseLong(data[1]) * 50;
			closingTimeout = Long.parseLong(data[2]) * 50;
	...
	private void resetTimeout() {
		...
		backgroundTimer.schedule(heartbeatTimeoutTask, closingTimeout
				+ heartbeatTimeout, closingTimeout + heartbeatTimeout);
	}
	...
	public void transportDisconnected() {
		...
		ChatEngine.socketerr_flag = true;
	}
	...
	public void transportError(Exception error) {
		...
		setState(STATE_INTERRUPTED);
		//reconnect();//pppppppppppppppppppppppppp
		Log.d("SSSSSSSSSSSSSSSSSS", "ssssssssssssssssss");
	}
